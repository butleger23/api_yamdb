...
TASKS = [
    ...
    ("category.csv", Category, {"id": int, "name": str, "slug": str}, None),
    ("genre.csv", Genre, {"id": int, "name": str, "slug": str}, None),
    (
        "titles.csv",
        Title,
    ...
    (
        "genre_title.csv",
        GenreTitle,
        {"id": int, "title_id": int, "genre_id": int},
        None,
    ),
    (
        "review.csv",
...
class Command(BaseCommand):
    ...
    def import_model(
        self, file_path, model_class, cast_fields, related_fields
    ):
        try:
    ...
    def general_mapper(
        self, row, model_class, cast_fields=None, related_fields=None
    ):
        try:
            instance_data = {}
            ...
            if related_fields:
                for field, (
                    ...
                    instance_data[field] = related_model.objects.get(
                        **{lookup_field: row[field]}
                    )
